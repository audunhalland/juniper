...
impl<'r, S: 'r> Registry<S> {
    ...
    pub fn get_type<T>(&mut self, info: &T::TypeInfo) -> Type<ArcStr>
    where
    ...
    fn insert_placeholder(&mut self, name: Name, of_type: Type<ArcStr>) {
        self.types
            .entry(name)
    ...
    pub fn build_union_type<T>(&mut self, info: &T::TypeInfo, types: &[Type<ArcStr>]) -> UnionMeta
    where
        T: GraphQLType<S> + ?Sized,
        S: ScalarValue,
    {
